...
2. **Corpus.** Get language Y's NT (or whatever Scripture exists) as verse-keyed JSON at `paths.corpus` — `paratext-import.ts` from a Paratext project, or `extract-reference-versions.ts` from a translation DB. Then run `bun tools/draft-readiness.ts`: it tells you honestly whether this is enough data to draft with, per genre, or whether to start in elicitation mode (`elicitation/ELICITATION.md`). Register every *other* scrap of language data the team has — dictionaries, grammars (even old ones), articles, field notes — via `register-source.ts` with a quality grade (`sources/README.md`).
